...
opXML::~opXML()
{
	...
	//获得应用程序路径
	 ...
	 wchar_t * opXML::ANSIToUnicode(const char* str)
	 {
		 int    textlen;
		 wchar_t * result;
	 ...
	 char * opXML::UnicodeToANSI(const wchar_t *str)
	 {
		 char * result;
		 int textlen;
	 ...
	 TCHAR * opXML::UTF8ToUnicode(const char* str)
	 {
		 int    textlen;
		 TCHAR * result;
		 textlen = MultiByteToWideChar(CP_UTF8, 0, str, -1, NULL, 0);
		 result = (TCHAR *)malloc((textlen + 1) * sizeof(wchar_t));
		 memset(result, 0, (textlen + 1) * sizeof(wchar_t));
		 MultiByteToWideChar(CP_UTF8, 0, str, -1, (LPWSTR)result, textlen);
		 return    result;
	 }

	 char * opXML::UnicodeToUTF8(const wchar_t *str)
	 {
		 char * result;
		 int textlen;
		 // wide char to multi char
